...
class Suite
  ...
  def run(selected_cases, output: $stdout, quiet: false)
    ...
    selected_cases.each do |bench|
      ...
      unless quiet
        output.puts(
          "%-42s n=%-8d total=%10.3fms per=%10.6fms" %
            [
              result[:name],
              result[:iterations],
              result[:total_ms],
...
OptionParser
  .new do |parser|
    ...
    parser.on(
      "--scale SCALE",
      Float,
      "Multiply iteration counts; default: BENCH_SCALE or 1.0"
    ) { |value| options[:scale] = value }

    parser.on(
      "--warmup COUNT",
      Integer,
      "Warmup iterations per case; default: BENCH_WARMUP or 5"
    ) { |value| options[:warmup] = value }

    parser.on(
      "--rounds COUNT",
      Integer,
      "Timed samples per case; default: BENCH_ROUNDS or 1"
    ...
    parser.on(
      "--compare PATH",
      "Compare current run against JSON results from PATH"
    ) { |value| options[:compare] = value }
...
ruby_array_of_hashes_1k =
  Array.new(1_000) { |i| { "id" => i, "name" => "x#{i}" } }
...
suite.add("js_to_ruby/return_10k_floats", 100) do
  ctx.call("floatArray", 10_000)
end
suite.add("js_to_ruby/return_10k_ascii_strings", 100) do
  ctx.call("asciiStringArray", 10_000)
end
suite.add("js_to_ruby/return_10k_latin1_strings", 50) do
  ctx.call("latin1StringArray", 10_000)
end
suite.add("js_to_ruby/return_10k_utf8_bmp_strings", 50) do
  ctx.call("utf16StringArray", 10_000)
end
suite.add("js_to_ruby/return_10k_utf8_emoji_strings", 50) do
  ctx.call("emojiStringArray", 10_000)
end
suite.add("js_to_ruby/return_object_1k_ints", 200) do
  ctx.call("objectOfInts", 1_000)
end
suite.add("js_to_ruby/return_array_1k_objects", 100) do
  ctx.call("arrayOfObjects", 1_000)
end
...
suite.add("ruby_to_js/roundtrip_10k_ints", 100) do
  ctx.call("id", ruby_ints_10k)
end
suite.add("ruby_to_js/roundtrip_10k_floats", 100) do
  ctx.call("id", ruby_floats_10k)
end
suite.add("ruby_to_js/roundtrip_10k_strings", 50) do
  ctx.call("id", ruby_strings_10k)
end
suite.add("ruby_to_js/roundtrip_10k_utf8_bmp_strings", 50) do
  ctx.call("id", ruby_utf8_strings_10k)
end
suite.add("ruby_to_js/roundtrip_10k_utf8_emoji_strings", 50) do
  ctx.call("id", ruby_emoji_strings_10k)
end
suite.add("ruby_to_js/roundtrip_hash_1k", 100) { ctx.call("id", ruby_hash_1k) }
suite.add("ruby_to_js/roundtrip_array_1k_hashes", 50) do
  ctx.call("id", ruby_array_of_hashes_1k)
end

# JS -> Ruby callback path. Each iteration performs 1,000 callbacks.
suite.add("callback/1000_echo_int", 50) do
  ctx.call("callRubyEchoMany", 1_000, 123)
end
suite.add("callback/1000_echo_short_string", 50) do
  ctx.call("callRubyEchoMany", 1_000, "hello")
end
suite.add("callback/1000_echo_utf8_string", 50) do
  ctx.call("callRubyEchoMany", 1_000, "Ā hello")
end
suite.add("callback/1000_echo_emoji_string", 50) do
  ctx.call("callRubyEchoMany", 1_000, "😀 hello")
end
...
if options[:compare]
  ...
  baseline_by_name =
    baseline.fetch(:benchmarks).to_h { |row| [row.fetch(:name), row] }
  comparisons =
