...
JOBS = [
  Job.new(name: "serde/default", tags: %w[all serde boundary default], script: "benchmark/serde/bench.rb", args: []),
  Job.new(name: "serde/single-threaded", tags: %w[all serde boundary single-threaded single], script: "benchmark/serde/bench.rb", args: ["--single-threaded"]),
  Job.new(name: "transpile/default", tags: %w[all transpile realworld default], script: "benchmark/transpile/bench.rb", args: []),
  Job.new(name: "transpile/single-threaded", tags: %w[all transpile realworld single-threaded single], script: "benchmark/transpile/bench.rb", args: ["--single-threaded"])
].freeze
...
OptionParser.new do |parser|
  ...
  parser.on("--tags TAGS", "Comma-separated benchmark tag filter; default: all") { |v| options[:tags] = v.split(",").map(&:strip).reject(&:empty?) }
  parser.on("--git-tags REFS", "Comma-separated git tags/refs to benchmark; use 'working' for this checkout") { |v| options[:git_refs] = v.split(",").map(&:strip).reject(&:empty?) }
  parser.on("--git-refs REFS", "Alias for --git-tags; accepts tags, branches, SHAs, and 'working'") { |v| options[:git_refs] = v.split(",").map(&:strip).reject(&:empty?) }
  parser.on("--worktree-root PATH", "Temp worktree root for --git-tags; default: tmp/benchmark/worktrees") { |v| options[:worktree_root] = v }
  parser.on("--[no-]setup-git-refs", "For git refs, run bundle install + rake compile; default: true") { |v| options[:setup_git_refs] = v }
  parser.on("--list", "List jobs/tags and exit") { options[:list] = true }
  parser.on("--quick", "Short smoke run: serde scale 0.1, transpile iterations 1, rounds 1") { options[:quick] = true }
  parser.on("--rounds COUNT", Integer, "Timed samples per benchmark case") { |v| options[:rounds] = v }
  parser.on("--warmup COUNT", Integer, "Warmup iterations per benchmark case") { |v| options[:warmup] = v }
  parser.on("--only REGEX", "Pass case-name regex through to selected suites") { |v| options[:only] = v }
  parser.on("--serde-scale SCALE", Float, "Scale serde iteration counts") { |v| options[:serde_scale] = v }
  parser.on("--transpile-iterations COUNT", Integer, "Iterations per transpile timed sample") { |v| options[:transpile_iterations] = v }
  parser.on("--transpile-source NAME", "Transpile resource name from benchmark/transpile/resources.json") { |v| options[:transpile_source] = v }
  parser.on("--transpile-cache-dir PATH", "Shared download cache for transpile resources") { |v| options[:transpile_cache_dir] = v }
  parser.on("--json", "Print machine-readable JSON") { options[:json] = true }
  parser.on("--output PATH", "Write JSON results to PATH") { |v| options[:output] = v }
end.parse!
...
abort "Unknown benchmark tag(s): #{unknown_tags.join(', ')}. Known tags: #{ALL_TAGS.join(', ')}" unless unknown_tags.empty?

selected_jobs = if options[:tags] == ["all"] || options[:tags].include?("all")
  JOBS
else
  JOBS.select { |job| (options[:tags] - job.tags).empty? }
end
abort "No benchmark jobs matched tags: #{options[:tags].join(', ')}" if selected_jobs.empty?
...
def prepare_target(ref, options)
  if WORKING_REFS.include?(ref)
    return Target.new(name: "working", ref: "working", dir: ROOT, working_tree: true)
  end

  FileUtils.mkdir_p(options[:worktree_root])
  dir = File.join(options[:worktree_root], safe_ref_name(ref))
  run_command!(%w[git worktree remove --force] + [dir], chdir: ROOT, quiet: true) if File.exist?(dir)
  FileUtils.rm_rf(dir)
  run_command!(%w[git worktree add --detach] + [dir, ref], chdir: ROOT, quiet: options[:json])
  sync_benchmark_files!(dir)

  if options[:setup_git_refs]
    warn "==> setting up #{ref}" unless options[:json]
    env = bundle_env(dir)
    run_command!([RbConfig.ruby, "-S", "bundle", "install"], chdir: dir, quiet: options[:json], env: env)
    run_command!([RbConfig.ruby, "-S", "bundle", "exec", "rake", "compile"], chdir: dir, quiet: options[:json], env: env)
  end
...
def command_for(job, options)
  ...
  when /\Atranspile\//
    args += ["--iterations", options[:transpile_iterations].to_s] if options[:transpile_iterations]
    args += ["--source", options[:transpile_source]] if options[:transpile_source]
    args += ["--cache-dir", options[:transpile_cache_dir]] if options[:transpile_cache_dir]
  end
...
def run_job(job, target, options)
  ...
  warn "==> #{target.name} #{job.name}: #{command.shelljoin}" unless options[:json]
  stdout, stderr, status = Bundler.with_unbundled_env do
...
target_results = targets.map do |target|
  {
    name: target.name,
    ref: target.ref,
    dir: target.dir,
...
def ascii_table(headers, rows, alignments = [])
  ...
  puts separator
  puts "| " + headers.each_with_index.map { |header, index| header.to_s.ljust(widths[index]) }.join(" | ") + " |"
  puts separator
  rows.each do |row|
    cells = row.each_with_index.map do |cell, index|
      alignment = alignments[index] || :left
      alignment == :right ? cell.to_s.rjust(widths[index]) : cell.to_s.ljust(widths[index])
    end
    puts "| " + cells.join(" | ") + " |"
  end
  puts separator
...
def print_single_target_job(job)
  ...
  rows = job.fetch(:benchmarks).map do |bench|
    [
      bench.fetch("name"),
  ...
  ascii_table(["benchmark", "ms/iter", "total ms", "iters", "rounds"], rows, [:left, :right, :right, :right, :right])
  puts
end

def print_comparison_job(job_name, target_results)
  targets = target_results.map { |target| target.fetch(:name) }
  jobs_by_target = target_results.to_h do |target|
    [target.fetch(:name), target.fetch(:jobs).find { |job| job.fetch(:name) == job_name }]
  end
  first_job = jobs_by_target.fetch(targets.first)
  first_metadata = first_job.fetch(:metadata)
  benchmark_names = first_job.fetch(:benchmarks).map { |bench| bench.fetch("name") }
  ...
  rows = benchmark_names.map do |benchmark_name|
    values = targets.to_h do |target|
      bench = jobs_by_target.fetch(target).fetch(:benchmarks).find { |row| row.fetch("name") == benchmark_name }
      [target, bench.fetch("ms_per_iter")]
    end
    best_target, best_value = values.min_by { |_target, value| value }
    slowest_value = values.values.max

    target_cells = targets.map do |target|
    ...
    [benchmark_name, *target_cells, best_target, format_pct((slowest_value / best_value - 1.0) * 100.0)]
  end
...
if target_results.length == 1
  ...
  target_results.each do |target|
    versions = target.fetch(:jobs).map { |job| job.fetch(:metadata)["mini_racer_version"] }.uniq
