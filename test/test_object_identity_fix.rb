...
class ObjectIdentityTest < Minitest::Test
  def test_separate_identical_objects_remain_separate
    ...
    assert_equal false,
                 obj1.equal?(obj2),
                 "Ruby objects should not be identical"
  ...
  def test_same_ruby_object_multiple_times_shares_identity
    ...
    assert_equal true,
                 objects[0].equal?(objects[1]),
                 "Should be same Ruby object"
    assert_equal true,
                 objects[0].equal?(objects[2]),
                 "Should be same Ruby object"
    ...
    assert_equal %w[shared shared shared],
                 result["final_ids"],
  ...
  def test_nested_objects_with_identical_content
    ...
    assert_equal "first",
                 result[0],
                 "First nested object should maintain separate identity"
    assert_equal "second",
                 result[1],
                 "Second nested object should maintain separate identity"
